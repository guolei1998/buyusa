...
class PayeezyHTTPAuthorize(object):
	...
	def generateHMACAuthenticationHeader(self, payload):
		...
		hmacInHex = hmac.new(self.apisecret.encode(), msg=messageData.encode(), digestmod=hashlib.sha256).hexdigest()
		return nonce,currentTimeInMilli,b64encode(hmacInHex.encode())
	...
	def getTokenPostCall(self, payload):
		...
		nonce,currentTimeInMilli,authorizationVal = self.generateHMACAuthenticationHeader(payload=self.payload)
		result = requests.post(self.tokenURL, headers={'User-Agent':'Payeezy-Python','content-type': 'application/json',
		                                               'apikey':self.apikey,'token':self.token,'Authorization':'xxxxx',
		                                               'nonce':nonce,'timestamp':currentTimeInMilli,
		                                               }, data=self.payload)
		return result
	...
	def makeCardBasedTransactionPostCall(self, payload):
		...
		self.payload = json.dumps(payload)
		nonce,currentTimeInMilli,authorizationVal = self.generateHMACAuthenticationHeader(payload=self.payload)
		result = requests.post(self.url, headers={'User-Agent':'Payeezy-Python','content-type': 'application/json',
		                                          'apikey':self.apikey,'token':self.token,'Authorization':authorizationVal,
		                                          'nonce':nonce,'timestamp':currentTimeInMilli,
		                                          }, data=self.payload)
		return result
